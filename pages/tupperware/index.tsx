            ...
            <Header>
                ...
                <div className='grid gap-4 sm:gap-6 grid-cols-2 sm:grid-cols-3 md:grid-cols-4 italic lg:grid-cols-5 my-6 mx-4 sm:mx-0'>
                    {products.map((product) => (
                        <Link href={`/tupperware/${product.slug}`} key={product.id}>
...
export const getServerSideProps: GetServerSideProps = async () => {
    ...
    const products = ProductsMapper(resultTupperware);

    return {
        props: {
            products,
            page: resultTupperware.page,
            totalPage: resultTupperware.total_pages,
        },
    };
};
